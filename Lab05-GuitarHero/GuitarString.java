public class GuitarString 
{
    ...
    public static final double decayFactor = 0.994;
    private int numTics;

    /** create a guitar string of the given frequency */
    public GuitarString(double frequency) {
        size = (int) (samplingRate/frequency);
    ...
    public void pluck() {
        ...
        for (int i = 0; i < buffer.size(); i++)
        {
            ...
            buffer.enqueue((random * 2 + before) / 3.0);
            before = (random * 2 + before) / 3.0;
        }
    }

    /** advance the simulation one time step */
    public void tic() {
        buffer.enqueue(((buffer.dequeue() + buffer.peek()) / 2) * decayFactor);
        numTics++;
    }
